package com.wezom.kiviremote.upnp.fi.iki.elonen;
...
public class SimpleWebServer extends NanoHTTPD {
        ...
        if (up != null || directories.size() + files.size() > 0) {
            ...
            if (!files.isEmpty()) {
                ...
                for (int i = 0; i < files.size(); i++) {
                    ...
                    builder.append("<li><a href=\"").append(encodeUri(uri + file)).append("\"><span class=\"filename\">").append(file).append("</span></a>");
                    File curFile = new File(f, file);
                    long len = curFile.length();
                    builder.append("&nbsp;<span class=\"filesize\">(");
                    if (len < 1024)
                        builder.append(len).append(" bytes");
                    else if (len < 1024 * 1024)
                        builder.append(len / 1024).append(".").append(len % 1024 / 10 % 100).append(" KB");
                    else
                        builder.append(len / (1024 * 1024)).append(".").append(len % (1024 * 1024) / 10 % 100).append(" MB");
                    builder.append(")</span></li>");
                }
                msg += builder.toString() + "</section>";
            }
            msg += "</ul>";
        }
        msg += "</body></html>";
    ...
    public Response serve(String uri, Method method, Map<String, String> header, Map<String, String> parms, Map<String, String> files) {
        if (!quiet) {
            ...
            while (e.hasNext()) {
                String value = e.next();
                Timber.d("  HDR: '" + value + separator + header.get(value) + "'");
            }
            e = parms.keySet().iterator();
            while (e.hasNext()) {
                String value = e.next();
                Timber.d("  PRM: '" + value + separator + parms.get(value) + "'");
            }
            e = files.keySet().iterator();
            while (e.hasNext()) {
                String value = e.next();
                Timber.d("  UPLOADED: '" + value + separator + files.get(value) + "'");
            }
        }
